...
export default function MarineSafetyPage() {
  ...
  const alertColor = 
    selectedZone.alertLevel === 'Red Alert' ? 'bg-red-500/20 text-red-200 border-red-500/40 shadow-xs' :
    selectedZone.alertLevel === 'Orange Advisory' ? 'bg-orange-500/20 text-orange-200 border-orange-500/40 shadow-xs' :
    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-300 text-white">
      ...
      <div className="glass-panel p-4 xs:p-5 sm:p-8 rounded-3xl border border-white/20 shadow-xl backdrop-blur-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-xl xs:text-2xl sm:text-3xl font-extrabold text-white tracking-tight">{t('marine_title', 'Marine Safety Advisory')}</h1>
            ...
            <button
              ...
              className={`p-3.5 xs:p-4 rounded-2xl border text-left transition-all cursor-pointer ${
                selectedZone.id === zone.id
                  ? 'bg-teal-500/25 border-teal-400 shadow-lg ring-2 ring-teal-400/30 scale-[1.02]'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="p-5 sm:p-6 rounded-3xl glass-panel border border-white/20 shadow-xl space-y-4 xs:space-y-5">
            ...
            <div className={`p-4 rounded-2xl border flex items-start gap-3 text-xs leading-relaxed ${
              selectedZone.alertLevel === 'Red Alert' ? 'bg-red-500/20 border-red-500/40 text-red-200' : 'bg-amber-500/20 border-amber-500/40 text-amber-200'
            }`}>
              <ShieldAlert className="w-5 h-5 flex-shrink-0 mt-0.5" />
              <div>
                <strong className="font-extrabold block text-sm mb-0.5">Maritime Safety Advisory:</strong>
                <span className="text-white/95 font-medium">{selectedZone.advisoryEn}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10">
                ...
                <div className="text-sm font-black text-white mt-1">{selectedZone.waveHeight}</div>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10">
                ...
                <div className="text-sm font-black text-white mt-1">{selectedZone.windSpeed}</div>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10">
                ...
                <div className="text-sm font-black text-white mt-1">{selectedZone.seaTemp}</div>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10">
                ...
                <div className="text-sm font-black text-white mt-1">{selectedZone.visibility}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="p-5 rounded-3xl glass-panel border border-white/20 shadow-xl space-y-3">
            <h3 className="font-extrabold text-sm text-white uppercase tracking-wider">Tide Schedule</h3>
            <div className="space-y-3">
              <div className="p-4 rounded-2xl bg-cyan-500/15 border border-cyan-400/35 flex items-center justify-between">
                <div>
                  <span className="text-xs font-extrabold text-cyan-300 uppercase">High Tide</span>
                  <div className="text-base font-black text-white mt-0.5">{selectedZone.highTide}</div>
                </div>
              ...
              <div className="p-4 rounded-2xl bg-white/10 border border-white/15 flex items-center justify-between">
                <div>
                  <span className="text-xs font-extrabold text-white/60 uppercase">Low Tide</span>
                  <div className="text-base font-black text-white mt-0.5">{selectedZone.lowTide}</div>
                </div>
                <Anchor className="w-6 h-6 text-white/60" />
              </div>
            </div>
          </div>
        </div>
