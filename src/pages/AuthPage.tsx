...
const AuthPage = () => {
  ...
  const selectGoogleAccount = (_name: string, _emailStr: string) => {
    setIsLoading(true);
        ...
        {showGoogleMock && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white text-slate-900 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden font-sans"
            >
              <div className="p-8 pb-6 flex flex-col items-center border-b border-slate-100">
                ...
                <h2 className="text-xl font-medium mb-1">Sign in with Google</h2>
                <p className="text-sm text-slate-600 text-center">Choose an account to continue to <span className="font-semibold text-slate-800">WeatherGPT AI</span></p>
              </div>
              <div className="flex flex-col p-2 bg-slate-50">
                <button 
                  onClick={() => selectGoogleAccount('Sharad Detroja', 'sharad.detroja@example.com')}
                  className="flex items-center gap-3 p-3 hover:bg-slate-100 rounded-xl transition-colors text-left"
                >
                  <div className="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold flex-shrink-0">
                    SD
                  </div>
                  <div>
                    <div className="font-medium text-slate-900 text-sm">Sharad Detroja</div>
                    <div className="text-xs text-slate-500">sharad.detroja@example.com</div>
                  </div>
                </button>
                <button 
                  onClick={() => selectGoogleAccount('Guest User', 'guest@example.com')}
                  className="flex items-center gap-3 p-3 hover:bg-slate-100 rounded-xl transition-colors text-left"
                >
                  ...
                  <div>
                    <div className="font-medium text-slate-900 text-sm">Guest User</div>
                    <div className="text-xs text-slate-500">guest@example.com</div>
                  </div>
                </button>
                <button 
                  onClick={() => setShowGoogleMock(false)}
                  className="mt-2 p-3 text-sm font-medium text-slate-500 hover:bg-slate-100 rounded-xl transition-colors text-center"
                >
                  Cancel
                </button>
              </div>
